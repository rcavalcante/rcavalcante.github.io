...
```{r LoadRunningData, eval=FALSE, echo=TRUE, message=FALSE, warning=FALSE}
library(DESeq2)
...
```{r FitModelStandard, message=FALSE, eval=FALSE}
# Apply model - takes some time to run
dds <- DESeq(dds)
```
```{r FitModelStandardCheck1, eval=FALSE}
resultsNames(dds)
```
...
```{r WriteOut.RData, eval=FALSE, echo=TRUE, message=FALSE, warning=FALSE}
